...
pub async fn export(
    ...
    match format.as_str() {
        "csv" => {
            let mut csv = String::from("checked_at,status,status_code,response_time_ms,error_message\n");
            for c in &checks {
            ...
            Ok(Response::builder()
                ...
                .header("Content-Disposition", &format!("attachment; filename=\"checks-{}.csv\"", id))
                .body(Body::from(csv))
                .unwrap())
        }
        "json" => {
            ...
            Ok(Response::builder()
                ...
                .header("Content-Disposition", &format!("attachment; filename=\"checks-{}.json\"", id))
                .body(Body::from(json))
                .unwrap())
        }
