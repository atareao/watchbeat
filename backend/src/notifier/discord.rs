...
mod tests {
    ...
    async fn test_discord_sends_correct_request() {
        ...
        let handle = tokio::spawn(async move {
            ...
            assert!(request.contains("\"content\":\"test alert\""));
        ...
        let result = send_discord_notification(&url, "test alert").await;
        assert!(result.is_ok());
        handle.await.unwrap();
    }
    ...
    async fn test_discord_connection_refused() {
        let result = send_discord_notification("http://127.0.0.1:1", "test message").await;
